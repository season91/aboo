...
public interface UsedRepository {
	...
	@Select("select count(*) from TB_USED_BRD where to_char(USED_REG_DATE,'YYYYMMDD') = to_char(sysdate,'YYYYMMDD') and APARTMENT_IDX =#{apartmentIdx}")
	int selectUsedBrdTodayCnt(String apartmentIdx);
	
	@Select("select count(*) from TB_INF_QST_BRD where to_char(B_WDATE,'YYYYMMDD') = to_char(sysdate,'YYYYMMDD') and APARTMENT_IDX =#{apartmentIdx}")
	int selectInfoBrdTodayCnt(String apartmentIdx);
	
	@Select("select count(*) from TB_INTERIOR_BRD where to_char(INT_REG_DATE,'YYYYMMDD') = to_char(sysdate,'YYYYMMDD') and APARTMENT_IDX =#{apartmentIdx}")
